...
class RegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        if y is not None:
            ...
            layer4 = nn.MatrixMultiply(graph, layer3, self.w2)
            layer5 = nn.MatrixVectorAdd(graph, layer4, self.b2)
            layer6 = nn.SquareLoss(graph, layer5, yInput)
            return graph
            ...
            layer4 = nn.MatrixMultiply(graph, layer3, self.w2)
            layer5 = nn.MatrixVectorAdd(graph, layer4, self.b2)
            return graph.get_output(layer5)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.learning_rate = 0.05
        self.h = 100
        self.w1_list = [nn.Variable(784, self.h) for digit in range(0,10)]
        self.b1_list = [nn.Variable(1, self.h) for digit in range(0, 10)]
        self.w2_list = [nn.Variable(self.h, self.h) for digit in range(0, 10)]
        self.b2_list = [nn.Variable(1, self.h) for digit in range(0, 10)]
        self.w3_list = [nn.Variable(self.h, 1) for digit in range(0, 10)]
        self.b3_list = [nn.Variable(1, 1) for digit in range(0, 10)]
