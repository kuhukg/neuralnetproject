...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return [self.valuesDict[x] for x in node.get_parents()]
    ...
    def add(self, node):
        ...
        value = node.forward(self.get_inputs(node))
        self.valuesDict[node] = value
        self.gradientDict[node] = np.zeros(value.shape)
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        self.gradientDict[loss_node] = np.asarray(1.0)
        
        for i in range(len(self.nodeList)-1, -1, -1):
            node = self.nodeList[i]
            backGradients = node.backward(self.get_inputs(node), self.get_gradient(node))
            counter = 0
            for parent in node.get_parents():
                self.gradientDict[parent] = self.gradientDict[parent]+backGradients[counter]
                counter+=1
    ...
    def step(self, step_size):
        ...
        for variable in self.variableSet:
            variable.data = variable.data-step_size*self.get_gradient(variable)
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.maximum(np.zeros_like(inputs[0]), inputs[0])

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.where(np.maximum(np.zeros_like(inputs[0]), inputs[0]) > 0, gradient, 0)]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.mean(0.5*np.subtract(inputs[0], inputs[1])*np.subtract(inputs[0], inputs[1]))

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [(np.subtract(inputs[0], inputs[1]) * gradient) / inputs[0].size, -(np.subtract(inputs[0], inputs[1]) * gradient) / inputs[1].size]
        
class ScalarMultiply(FunctionNode):
